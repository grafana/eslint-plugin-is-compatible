...
export function getMinSupportedVersionFromPackageJson(): string {
  ...
  if (!fs.existsSync(path)) {
    throw new Error(
      "Couldn't find src/plugin.json in the current working directory"
    );
  }
...
export function getMinSupportedGrafanaVersion(
  context: Readonly<RuleContext<"issue:import", Options>>
) {
  if (
    context.options.length &&
    context.options[0].minGrafanaVersion !== undefined
  ) {
